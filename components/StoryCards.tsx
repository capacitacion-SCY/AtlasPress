...
export function FeatureStoryCard({ story, index = 0 }: { story: Story; index?: number }) {
  ...
  return (
    <article className={`feature-story feature-story--landscape ${sideClass}`} data-category={story.categories?.slug}>
      ...
      <Link className="feature-story__media" href={`/article/${story.slug}`}>
        {story.image_url ? (
          <img className="feature-story__img" src={story.image_url} alt={story.title} />
        ) : (
          <span className="feature-story__fallback">Imagen no disponible</span>
        )}
      </Link>
  ...
  return (
    <article className="compact-story" data-category={story.categories?.slug}>
      ...
      <Link className="compact-story__thumb" href={`/article/${story.slug}`}>
        {story.image_url ? (
          <img className="compact-story__img" src={story.image_url} alt={story.title} />
        ) : (
          <span className="compact-story__fallback">Foto</span>
        )}
      </Link>
